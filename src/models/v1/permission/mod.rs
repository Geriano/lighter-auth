...
use crate::entities::v1::permissions::{ActiveModel, Column, Entity, Model};
...
impl Model {
    pub async fn find_by_id(
        ...
        if let Some(m) = metrics {
            m.record_db_query("permission_find_by_id", start.elapsed().as_secs_f64());
        }
    ...
    pub async fn code_exist<T: ToString>(
        ...
        if let Some(m) = metrics {
            m.record_db_query("permission_code_exist", start.elapsed().as_secs_f64());
        }
    ...
    pub async fn store(
        ...
        if let Some(m) = metrics {
            m.record_db_query("permission_store", start.elapsed().as_secs_f64());
        }
    ...
    pub async fn update<T: ToString>(
        ...
        if let Some(m) = metrics {
            m.record_db_query("permission_update", start.elapsed().as_secs_f64());
        }
    ...
    pub async fn delete(
        ...
        if let Some(m) = metrics {
            m.record_db_query("permission_delete", start.elapsed().as_secs_f64());
        }
